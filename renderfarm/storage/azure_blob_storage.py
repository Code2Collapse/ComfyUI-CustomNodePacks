...
Env:
    ...
    RIB_AZURE_CONTAINER             (required)  container name
    RIB_AZURE_URL_TTL               (optional)  SAS GET lifetime seconds, default 86400
"""
...
class AzureBlobStorage(BaseStorage):
    ...
    def __init__(self, service_client=None):
        conn, self.container = require_env("AZURE_STORAGE_CONNECTION_STRING",
                                           "RIB_AZURE_CONTAINER")
        self.ttl = int(os.environ.get("RIB_AZURE_URL_TTL", "86400"))
        try:
            from azure.storage.blob import BlobServiceClient
        except ImportError as exc:
            raise RuntimeError(
                "RIB storage: azure-storage-blob is not installed. "
