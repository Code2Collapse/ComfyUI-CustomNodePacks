...
Env:
    RIB_GCS_BUCKET                 (required)  bucket name
    GOOGLE_APPLICATION_CREDENTIALS (required)  service-account JSON path
    RIB_GCS_URL_TTL                (optional)  signed GET lifetime seconds, default 86400
"""
...
class GCSStorage(BaseStorage):
    ...
    def __init__(self, client=None):
        (self.bucket_name,) = require_env("RIB_GCS_BUCKET")
        require_env("GOOGLE_APPLICATION_CREDENTIALS")
        self.ttl = int(os.environ.get("RIB_GCS_URL_TTL", "86400"))
        ...
        try:
            ...
            raise RuntimeError(
                "RIB storage: google-cloud-storage is not installed. "
                "Run: pip install google-cloud-storage"
            ) from exc
        self.client = gcs.Client()

    def upload(self, local_path: str) -> str:
        blob_name = f"rib/{uuid.uuid4().hex[:10]}/{os.path.basename(local_path)}"
