/**
 * mec_tensor_inspector.js — Phase 4: Live Tensor Inspector
 *
...
function _renderPanel(panel, nodeTitle, nodeId, data) {
    if (!data || !data.available) {
        ...
        panel.innerHTML = `
            <div class="ti-header">
                <span class="ti-title">🔬 ${_esc(nodeTitle)} <span style="color:#6c7086;font-weight:400;">#${_esc(nodeId)}</span></span>
                <button class="ti-close">×</button>
            </div>
...
async function _showFor(node, slotIndex, mouseX, mouseY) {
    ...
    panel.innerHTML = `
        <div class="ti-header">
            <span class="ti-title">🔬 ${_esc(title)} <span style="color:#6c7086;font-weight:400;">#${_esc(nodeId)}</span></span>
            <button class="ti-close">×</button>
        </div>
...
app.registerExtension({
    name: "MEC.TensorInspector",
    settings: [
        {
            id: "mec.tensor_inspector.enabled",
    ...
    async setup() {
        ...
        const enabled = (() => {
            try { return app.ui.settings.getSettingValue("mec.tensor_inspector.enabled", true); }
            catch { return true; }
        })();
        if (!enabled) return;
