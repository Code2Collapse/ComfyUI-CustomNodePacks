...
const PREDICATES = {
    ...
    trimap_dilate:        (s, m) => m && m !== "none",
    trimap_erode:         (s, m) => m && m !== "none",
    edge_radius:          (s, m) => m && m !== "none",
...
function refreshVisibility(node) {
    ...
    for (const [name, pred] of Object.entries(PREDICATES)) {
        ...
        if (!w) continue;
        const visible = !!pred(seg, mat, mode, sup);
        setHidden(w, !visible);
    }
